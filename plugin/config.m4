...
if test "$PHP_CUDA" != "no"; then
    ...
    if test "$PHP_NVRTC" != "no"; then
        ...
        if test -f "$NVRTC_DIR/include/nvrtc.h"; then
            ...
            dnl Driver API (libcuda) lives with the driver; the toolkit ships stubs for linking.
            if test -d "$CUDA_LIBDIR/stubs"; then
                PHP_ADD_LIBRARY_WITH_PATH(cuda, $CUDA_LIBDIR/stubs, CUDA_SHARED_LIBADD)
            else
                PHP_ADD_LIBRARY_WITH_PATH(cuda, $CUDA_LIBDIR, CUDA_SHARED_LIBADD)
            fi
